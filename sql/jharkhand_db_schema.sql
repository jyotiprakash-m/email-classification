-- Jharkhand Procurement Database Schema
-- Database: jharkhand_db_simulation
-- Complete Database Creation Script

-- Create database
CREATE DATABASE IF NOT EXISTS jharkhand_db_simulation;
USE jharkhand_db_simulation;
...
CREATE TABLE user (
    ...
    office_address TEXT,
    user_role ENUM('Admin', 'Applicant', 'Approver', 'Vendor', 'Delivery_Agent') NOT NULL,
    approval_level INT DEFAULT NULL, -- For approvers, defines their approval hierarchy level
...
CREATE TABLE applications (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (applicant_user_id) REFERENCES user(user_id) ON DELETE RESTRICT
);
...
-- Table for storing approval workflow
CREATE TABLE approvals (
    ...
    FOREIGN KEY (approver_user_id) REFERENCES user(user_id) ON DELETE RESTRICT
);
...
-- Table for work orders and purchase orders
CREATE TABLE orders (
    order_id INT PRIMARY KEY AUTO_INCREMENT,
    application_id INT NOT NULL,
    order_number VARCHAR(50) UNIQUE NOT NULL,
    order_type ENUM('Work Order', 'Purchase Order') NOT NULL,
    ...
    status ENUM('Draft', 'Issued', 'In Progress', 'Completed', 'Cancelled') DEFAULT 'Draft',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (application_id) REFERENCES applications(application_id) ON DELETE CASCADE
);

-- Table for storing item details (for purchase orders)
...
CREATE TABLE deliveries (
    ...
    delivery_agent_user_id INT NULL,
    delivery_date DATE,
    delivery_address TEXT NOT NULL,
    delivery_contact VARCHAR(255),
    tracking_number VARCHAR(100),
    delivery_status ENUM('Pending', 'In Transit', 'Delivered', 'Failed', 'Returned') DEFAULT 'Pending',
    delivery_notes TEXT,
    ...
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (order_id) REFERENCES orders(order_id) ON DELETE CASCADE,
    FOREIGN KEY (delivery_agent_user_id) REFERENCES user(user_id) ON DELETE SET NULL,
...
CREATE TABLE documents (
    ...
    reference_type ENUM('Application', 'Approval', 'Order', 'Delivery') NOT NULL,
    ...
    FOREIGN KEY (uploaded_by_user_id) REFERENCES user(user_id) ON DELETE RESTRICT
);
...
CREATE INDEX idx_application_applicant ON applications(applicant_user_id);

-- Approval table indexes
CREATE INDEX idx_approval_status ON approvals(approval_status);
CREATE INDEX idx_approval_application ON approvals(application_id);
CREATE INDEX idx_approval_user ON approvals(approver_user_id);

-- Order table indexes
CREATE INDEX idx_order_status ON orders(status);
CREATE INDEX idx_order_date ON orders(order_date);
CREATE INDEX idx_order_application ON orders(application_id);
...
CREATE INDEX idx_document_reference ON documents(reference_type, reference_id);
CREATE INDEX idx_document_uploaded_by ON documents(uploaded_by_user_id);
...
-- Insert sample users
INSERT INTO user (username, email, password_hash, first_name, last_name, phone, employee_id, designation, department, user_role, approval_level, is_active) VALUES
...
INSERT INTO applications (application_number, applicant_user_id, application_type, description, estimated_cost, priority_level) VALUES
...
INSERT INTO approvals (application_id, approver_user_id, approval_level, approval_status, approval_date, comments) VALUES
...
INSERT INTO orders (application_id, order_number, order_type, vendor_name, vendor_contact, order_amount, order_date, expected_completion_date, status) VALUES
...
INSERT INTO order_items (order_id, item_name, item_description, quantity, unit_price, total_price, specifications) VALUES
...
INSERT INTO deliveries (order_id, delivery_number, delivery_agent_user_id, delivery_address, delivery_contact, delivery_status, delivery_notes) VALUES
...
INSERT INTO audit_log (table_name, record_id, action, old_values, new_values, changed_by_user_id, changed_at) VALUES
...
-- View for application summary with user details
...
SELECT 
    ...
    a.application_date
FROM applications a
JOIN user u ON a.applicant_user_id = u.user_id;

-- View for pending approvals with user details
...
JOIN user applicant ON a.applicant_user_id = applicant.user_id
JOIN user approver ON ap.approver_user_id = approver.user_id
WHERE ap.approval_status = 'Pending';

-- View for delivery tracking
CREATE VIEW v_delivery_tracking AS
SELECT 
    d.delivery_id,
    d.delivery_number,
    o.order_number,
    o.order_type,
    a.application_number,
    CONCAT(agent.first_name, ' ', agent.last_name) AS delivery_agent,
    d.delivery_address,
    d.delivery_status,
    d.delivery_date,
    d.tracking_number
...
LEFT JOIN user agent ON d.delivery_agent_user_id = agent.user_id;
...
BEGIN
    ...
    -- Get user's applications if they are an applicant
    SELECT 
        application_number,
        application_type,
        description,
        ...
        application_date
    FROM applications 
    WHERE applicant_user_id = user_id_param
    ORDER BY application_date DESC
    LIMIT 10;
    
    -- Get pending approvals if they are an approver
    SELECT 
        a.application_number,
        CONCAT(u.first_name, ' ', u.last_name) AS applicant_name,
        a.description,
    ...
    JOIN applications a ON ap.application_id = a.application_id
    JOIN user u ON a.applicant_user_id = u.user_id
    WHERE ap.approver_user_id = user_id_param 
    AND ap.approval_status = 'Pending'
...
-- =============================================================================
SELECT 'Jharkhand Procurement Database successfully created!' AS status;